...
import os
from datetime import datetime
from app import create_app  # Ensure this points to your __init__.py factory
from app.extensions import db
from app.models.case import Case
from app.models.casefile import CaseFile

# Create an app instance to be used by the worker
app = create_app()

def analyze_memory_dump(case_id: int):
    """
    Analyze a memory dump file for a given case within the Flask application context.
    """
    
    # WRAP EVERYTHING IN THE APP CONTEXT
    with app.app_context():
        ...
        try:
            ...
            # Update case status to FAILED
            try:
                # Re-fetch case to ensure we have a fresh session object
                case = Case.query.get(case_id)
                if case:
                    case.status = 'failed'
                    case.updated_at = datetime.utcnow()
